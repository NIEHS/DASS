...
observeEvent(input$compareToRef, {
  ...
  # If reference is another DA output
    ...
    data_compare <- data.table(
      usr_dt()[,input$idColumnsRes, with = F],
      dass_res$results[,.(ref = get(input$perfRefRes),
  ...
  names(compRes) <- gsub("([[:lower:]]|[0-9])([[:upper:]])", "\\1 \\2", names(compRes)) |>
    gsub("^(.){1}", "\\U\\1", x = _, perl = T)
  
  cmLoc <- grep_ci("confusion", names(compRes))
  
  metricTable <- data.table(
    Metric = names(compRes)[-cmLoc],
    Value = unlist(compRes[-cmLoc])
  )
})
